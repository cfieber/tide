...
trait EddaService {
  ...
  @Headers(Array("Accept: application/json"))
  @GET("/aws/securityGroups;_expand:(groupId,groupName,vpcId)")
  def securityGroupIdentifiers: List[SecurityGroup]
  ...
  def loadBalancers: List[LoadBalancer]
  ...
  def launchConfigurations: List[LaunchConfiguration]
  ...
  def autoScalingGroups: List[AutoScalingGroup]
