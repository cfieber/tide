...
class MainToVpc0Transformation extends VpcTransformation{
  ...
  def translateIpPermissions(reference: AwsReference[SecurityGroupIdentity], securityGroupState: SecurityGroupState): Set[IpPermission] = {
    val convertedIpPermissions = securityGroupState.ipPermissions.map { ipPermission =>
      ...
      ipPermission.ipRanges.foreach { ipRange =>
        ...
        matchRemovePatterns match {
          case Some(it) =>
            log = s"Did not copy IP range $ipRange from ${reference.akkaIdentifier} because 10. and 100. rules are redundant in VPC 0." :: log
          case None =>
            newIpRanges += ipRange
        }
        mainVpcNatIpsToLocation.get(ipRange) match {
          case Some(locationForNatIp) =>
            val netNAT = vpc0VpcNatIpsByLocation(locationForNatIp)
            log = s"Added NAT IP Range $netNAT when copying ${reference.akkaIdentifier}." :: log
            newIpRanges += netNAT
          case None =>
            Nil
        }
